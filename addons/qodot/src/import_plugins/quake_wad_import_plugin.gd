...
func import(source_file, save_path, options, r_platform_variants, r_gen_files):
	...
	var palette_path = options['palette_file']
	var palette_file = load(palette_path) as QuakePaletteFile
	if not palette_file:
	...
	var magic_string = magic.get_string_from_ascii()
	...
	for entry_idx in range(0, num_entries):
		...
		var in_wad_size = file.get_32()
		var size = file.get_32()
		var type = file.get_8()
		var compression = file.get_8()
		var unknown = file.get_16()
		var name = file.get_buffer(TEXTURE_NAME_LENGTH)
		var name_string = name.get_string_from_ascii()
	...
	for texture_data in texture_data_array:
		var name = texture_data[0]
		var width = texture_data[1]
		var height = texture_data[2]
		var pixels = texture_data[3]

		var pixels_rgb = PoolByteArray()
		for palette_color in pixels:
			var rgb_color = palette_file.colors[palette_color]
