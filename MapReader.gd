...
func read_map_file(file: File, options: Dictionary = {}) -> QuakeMap:
	...
	while(parse):
		...
		if(line == null):
			...
			map_entities.append(read_entity(file, options))
	
	return QuakeMap.new(map_entities)

func read_entity(file: File, options: Dictionary = {}) -> QuakeEntity:
	...
	while(parse):
		...
		if(line == null):
			...
			entity_brushes.append(read_brush(file, options))
		elif(line_starts_with(line, "}")):
...
func read_brush(file: File, options: Dictionary = {}) -> QuakeBrush:
	...
	while(parse):
		...
		if(line == null):
			...
			brush_planes.append(parse_plane(line, options))
		elif(line_starts_with(line, "}")):
...
func parse_plane(line: String, options: Dictionary = {}) -> QuakePlane:
